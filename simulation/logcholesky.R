...
for(it in 1:itermax){
  ...
  #generate data Y, all data are isometrically transformed to R^{3}
  Z1=runif(n,-0.1,0.1);
  Z2=runif(n,-0.1,0.1);
  Z3=runif(n,-0.1,0.1);
  Y=array(0,c(n,mmax,3));
  for(i in 1:n){
    for(j in 1:m[i]){
      vare=runif(1,-0.025,0.025);
      Y[i,j,1]=t[i,j]+t[i,j]*Z1[i]+vare;
      Y[i,j,2]=t[i,j]+t[i,j]*Z2[i]+vare;
      Y[i,j,3]=t[i,j]+t[i,j]*Z3[i]+vare;
    }
  }
...
name=paste("logcholesky_n",n,"_mp",mp,".txt",sep="");
